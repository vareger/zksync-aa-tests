...
describe("Paymaster`s work test", function(){
    ...
    it("Deploy Paymaster", async function(){         
        ...
        const paymasterArtifact = await deployer.loadArtifact('PaymasterEstimateGasError');
    ...
    it('Use Paymaster for regular contract function', async function(){
        let paymasterParams = await preparePaymasterParams(0, emptyWallet.address, PAYMASTER_ADDRESS);
        
        let gasPrice = await provider.getGasPrice();
        console.log(`Regular contract function gasPrice = ${gasPrice}`);

        let value = 11;

        let gasLimit = await testContract.estimateGas.regularContractFunction(value, {
            customData: {
        ...
        console.log(`Regular contract function gasLimit = ${gasLimit}`);
        
        let fee = gasPrice.mul(gasLimit.toString());
        console.log("Regular contract function fee = ", ethers.utils.formatEther(fee));
        let valueToSend = fee.add(ethers.utils.parseEther('0.0001'));
        
        await fundPaymaster(hre, provider, valueToSend, PAYMASTER_ADDRESS);
        
        await (
            await testContract.regularContractFunction(value, {
        ...
        expect(await testContract.value()).to.be.eq(value);
        console.log("Regular contract function succeded!");
    })

    it('Try to get paymaster`s value variable through TestContract`s functions', async function(){
        let paymasterParams = await preparePaymasterParams(0, emptyWallet.address, PAYMASTER_ADDRESS);
    
        let gasPrice = await provider.getGasPrice();
        console.log(`checkPaymasterValue function gasPrice = ${gasPrice}`);
        
        let gasLimit = await testContract.estimateGas.checkPaymasterValue(PAYMASTER_ADDRESS, {
            customData: {
                ergsPerPubdata: utils.DEFAULT_ERGS_PER_PUBDATA_LIMIT,
                paymasterParams: {
                    paymaster: paymasterParams.paymaster,
        ...
        // console.log("Can not estimate gas for function which wants to get data from paymaster");

        console.log(`checkPaymasterValue function gasLimit = ${gasLimit}`);
        
        let fee = gasPrice.mul(gasLimit.toString());
        console.log("checkPaymasterValue function fee = ", ethers.utils.formatEther(fee));
        let valueToSend = fee.add(ethers.utils.parseEther('0.0001'));
        
        await fundPaymaster(hre, provider, valueToSend, PAYMASTER_ADDRESS);

        await (
            await testContract.checkPaymasterValue(PAYMASTER_ADDRESS, {
            // provide gas params manually
            maxFeePerGas: gasPrice,
            maxPriorityFeePerGas: gasPrice,
            gasLimit,
        ...
        expect(await testContract.value()).to.be.eq(await paymaster.value());
        console.log("checkPaymasterValue function succeded!");
    })
